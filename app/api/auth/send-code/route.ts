...
export async function POST(req: NextRequest) {
  ...
  // LOCAL DEV: fixed code, no email (bypass Resend until the sending domain is verified).
  // NEVER set AUTH_DEV_CODE in production.
  const devCode = process.env.AUTH_DEV_CODE;
  const code = devCode || String(crypto.getRandomValues(new Uint32Array(1))[0] % 1_000_000).padStart(6, '0');

  if (!devCode) {
    if (!resendKey) return NextResponse.json({ error: 'Auth is not configured on the server.' }, { status: 500 });
